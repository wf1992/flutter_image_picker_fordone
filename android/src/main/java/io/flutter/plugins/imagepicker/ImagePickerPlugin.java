...
public class ImagePickerPlugin implements MethodChannel.MethodCallHandler {
  ...
  public void onMethodCall(MethodCall call, MethodChannel.Result result) {
    ...
    Fragment fragment = ((FragmentActivity)registrar.activity()).getSupportFragmentManager().getFragments().get(0); //只试用当前页面嵌套FlutterFragment中调用imagepicker组件
    if (call.method.equals("pickImage")) {
      int imageSource = call.argument("source");
      switch (imageSource) {
        case SOURCE_GALLERY:
          delegate.chooseImageFromGallery(call, result, fragment);
          break;
        case SOURCE_CAMERA:
          delegate.takeImageWithCamera(call, result, fragment);
          break;
        default:
      ...
      switch (imageSource) {
        case SOURCE_GALLERY:
          delegate.chooseVideoFromGallery(call, result, fragment);
          break;
        case SOURCE_CAMERA:
          delegate.takeVideoWithCamera(call, result,fragment);
          break;
        default:
